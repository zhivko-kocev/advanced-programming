...
class LogSystem<E extends ILog> {
    ...
    private final ArrayList<E> logsList;

    public LogSystem(ArrayList<E> logsList) {
    ...
    void printResults() {
        ...
        LogProcessor<E> firstLogProcessor = l-> (ArrayList<E>) l.stream().filter(e->e.getType().equals("INFO")).collect(Collectors.toList());

        LogProcessor<E> secondLogProcessor = l-> (ArrayList<E>) l.stream().filter(m->m.getMessage().length()<100).collect(Collectors.toList());

        LogProcessor<E> thirdLogProcessor = l-> (ArrayList<E>) l.stream().sorted(Comparator.comparingLong(ILog::getTimestamp)).collect(Collectors.toList());
