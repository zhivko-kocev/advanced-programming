...
import java.util.*;
interface MergeStrategy<V extends Comparable<V>>{
    V merge(V obj1,V obj2);
}

class MapOps{
    public static <V extends Comparable<V>> Map<V,V> merge(Map<V,V> map1,Map<V,V> map2,MergeStrategy<V> mergeStrategy){
        Map<V,V> map3=new TreeMap<>();
        map1.keySet()
                .forEach(key -> {
                    map2.computeIfPresent(key,(key2,value) ->
                            map3.putIfAbsent(key,mergeStrategy.merge(map1.get(key),value)));
                    map3.putIfAbsent(key,map1.get(key));
                });
        map2.keySet()
                .forEach(key -> map3.putIfAbsent(key,map2.get(key)));
        return map3;
    }
}
...
public class GenericMapTest {
    public static void main(String[] args) {
        ...
        if (testCase == 1) { //Mergeable integers
            ...
            MergeStrategy<String> mergeStrategy = (obj1,obj2)->obj1+obj2;
            ...
            MergeStrategy<String> mergeStrategy = (obj1,obj2)-> obj1.replace(obj2,"*".repeat(obj2.length()));
            printMap(MapOps.merge(mapLeft, mapRight, mergeStrategy));
        }
    }
