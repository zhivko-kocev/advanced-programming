...
class GenericCollection<T extends IHasTimestamp & Comparable<T>> {
    ...
    private final Map<String, List<T>> elements;
    ...
    public Collection<T> itemsFromCategories(
        ...
        return elements.keySet().stream()
                ...
                .flatMap(category -> elements.get(category)
                        .stream())
                .collect(Collectors.toCollection(() -> new TreeSet<T>(
                        Comparator.reverseOrder())));
    }
    ...
    public Map<String, Set<T>> byMonthAndDay() {
        return elements.values().stream()
                .flatMap(Collection::stream)
                .sorted(Comparator.comparing((T t) -> t.getTimestamp().getMonth().getValue()).thenComparing(t -> t.getTimestamp().getDayOfMonth()))
